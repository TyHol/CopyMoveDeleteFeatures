...
Item {
    ...
    // Current operation mode: "delete" | "move" | "copy"
    property string _mode: "delete"

    // true = show filter controls; false = load all features (hide filter section)
    property bool   _useFilter: false
    ...
    Dialog {
        ...
        title: _mode === "move"
               ...
               : _mode === "copy" ? qsTr("Copy features — CopyMoveDeleteFeatures")
        ...
        onAccepted: {
            ...
            // OK always means checklist mode
            directMode = false
        ...
        ColumnLayout {
            ...
            StackLayout {
                ...
                ScrollView {
                    ...
                    ColumnLayout {
                        // Subtract scrollbar width when visible so it never overlaps content.
                        width: mainTabScrollView.width
                               - (mainTabScrollView.ScrollBar.vertical.visible
                                  ? mainTabScrollView.ScrollBar.vertical.width : 0)
                        spacing: 6
                        ...
                        // ── All / Filter toggle ──────────────────────────────
                        // Uses a distinct green-tinted style so these buttons are
                        // clearly separate from the Delete/Move/Copy row above.
                        RowLayout {
                            Layout.fillWidth: true
                            spacing: 4
                            Button {
                                ...
                                background: Rectangle {
                                    radius: 4
                                    color: allBtn.highlighted
                                           ? Theme.mainColor
                                           : Qt.rgba(Theme.mainColor.r,
                                                     Theme.mainColor.g,
                                                     Theme.mainColor.b, 0.12)
                                ...
                                contentItem: Text {
                                    text: allBtn.text
                                    font: allBtn.font
                                    color: allBtn.highlighted ? "white" : Theme.mainColor
                                    horizontalAlignment: Text.AlignHCenter
                                    verticalAlignment:   Text.AlignVCenter
                                    elide: Text.ElideRight
                                }
                                readonly property bool highlighted: !_useFilter
                                onClicked: {
                                    if (_useFilter) {
                                        _useFilter = false
                                        loadFeatures(null)
                            ...
                            Button {
                                ...
                                contentItem: Text {
                                    text: filterBtn.text
                                    font: filterBtn.font
                                    color: filterBtn.highlighted ? "white" : Theme.mainColor
                                    horizontalAlignment: Text.AlignHCenter
                                    verticalAlignment:   Text.AlignVCenter
                                    elide: Text.ElideRight
                                }
                        ...
                        // ── Filter section (shown when Filter is selected) ─────
                        ColumnLayout {
                            visible: _useFilter
                            Layout.fillWidth: true
                            spacing: 6

                            // Filter header + help button
                            RowLayout {
                                Layout.fillWidth: true
                                Label {
                                    text: qsTr("Filter")
                            ...
                            // ── Apply + Clear buttons (below value) ───────────
                            RowLayout {
                                Layout.fillWidth: true
                                spacing: 4
                                Button {
                            ...
                            // Hidden label kept for backward-compat with restoreLayerFilter etc.
                            Label {
                                id: exprPreviewLabel
                                text: ""
                                visible: false
                            ...
                            Label {
                                Layout.fillWidth: true
                                text: qsTr("⚠ Note: performance may be slow with large numbers of features.")
                                font: Theme.tipFont
                                color: Theme.secondaryTextColor
                                wrapMode: Text.Wrap
                                opacity: 0.8
    ...
    Dialog {
        ...
        title: {
            if (directMode)
                return _mode === "move" ? qsTr("Move ALL matching?")
                     : _mode === "copy" ? qsTr("Copy ALL matching?")
        ...
        ColumnLayout {
            ...
            Label {
                ...
                text: {
                    if (directMode) {
                        var verb = _mode === "move" ? qsTr("move")
                                 : _mode === "copy" ? qsTr("copy") : qsTr("delete")
                        var where = (_mode === "move" || _mode === "copy")
        ...
        onAccepted: {
            ...
            if (directMode) {
                ...
                if (_mode === "move" || _mode === "copy") {
                    ...
                    if (!dstLayerD) return
                    count = directCopyMoveByExpression(srcLayer, dstLayerD, directExpr, _mode === "move")
                    if (count > 0) {
                        var verbD = _mode === "move" ? qsTr("Moved") : qsTr("Copied")
    ...
    // If a saved filter exists, switches to Filter mode automatically.
    function restoreLayerFilter(layerName) {
        ...
        if (!saved || !saved.field) {
            // No saved filter — switch to All mode and load all features
            _useFilter = false
            valueField.text = ""
            exprField.text  = ""
            loadFeatures(null)
        ...
        // Has a saved filter — switch to Filter mode to show controls
        _useFilter = true
    ...
    function fetchSuggestions() {
        ...
        if (/^\w+\s*\(/.test(searchText)) return   // looks like a function call — skip suggestions
    ...
    function buildExpression() {
        ...
        // Must start with a word identifier then '(' — e.g. now(), today(), to_date(…)
        // Plain data values that happen to contain '(' are NOT treated as functions.
        ...
        if (op === "LIKE") {
            ...
            var inVals = parts.map(function(p) {
                ...
                var pIsFn  = /^\w+\s*\(/.test(p)   // e.g. to_date(…), now()
            ...
            ColumnLayout {
                ...
                Label {
                    ...
                    text: qsTr(
                        ...
                        "2. Tap Filter and set a filter to narrow which features are shown.\n" +
                        "3. Tick/untick features, or use All / None.\n" +
                        "4. Choose Delete, Move or Copy at the top.\n" +
                        "5. Type 'a' to confirm (7-second timeout).\n\n" +
                        "Filters are saved per layer and restored automatically next session. " +
                        "Press Clear Filter to remove a saved filter."
                    )
                }
